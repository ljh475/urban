...
const overviewItems: SimpleItem[] = [
  {
    title: '정비사업이란',
    desc: '도시기능 회복과 주거환경 개선을 위한 정비사업의 개념 정리',
    href: '#overview',
  },
  {
    title: '사업 유형',
    desc: '재개발, 재건축, 소규모정비사업의 특징과 차이 비교',
    href: '#overview',
  },
  {
    title: '용어 정리',
    desc: '조합, 추진위원회, 사업시행자, 관리처분 등 핵심 용어 설명',
    href: '#overview',
  },
];
...
const resourceItems: SimpleItem[] = [
  {
    title: '체크리스트',
    desc: '단계별 검토사항과 의사결정 포인트를 한 번에 확인',
    href: '#resources',
  },
  {
    title: '보고서 양식',
    desc: '공사비 검증, 사업 절차 정리, 회의자료용 문서 포맷',
    href: '#resources',
  },
  {
    title: 'PDF 자료',
    desc: '법령 요약, 절차 정리, 실무 참고자료 다운로드',
    href: '#resources',
  },
];
...
function StepCard({ title, href, steps }: ProcessCard) {
  return (
    <a
      href={href}
      className="block rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-slate-400"
    >
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <a
            href="#resources"
            className="rounded-2xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:scale-[1.02]"
          >
            자료 보기
          </a>
        </div>
          ...
          <div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-4 py-3 sm:px-6">
            {menu.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="whitespace-nowrap rounded-full border border-slate-200 px-3 py-2 text-xs font-medium text-slate-700"
              >
                {item.label}
              </a>
            ))}
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:py-28">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <p className="max-w-2xl text-base leading-7 text-slate-600 lg:text-lg">
                REB 참고 구조를 바탕으로 정비사업 개요, 절차, 공사비 검증, 관련 법령,
                자료실을 한곳에서 볼 수 있도록 구성한 정보형 웹사이트입니다.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href="#process"
                className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-300"
              >
                사업 절차 보기
              </a>
              <a
            ...
            <div className="grid gap-4 pt-4 sm:grid-cols-3">
              <ClickCard
                title="정비사업 핵심 구조"
                desc="재개발, 재건축, 소규모정비사업을 한눈에 비교"
                href="#overview"
              />
              <ClickCard
                title="공사비 검증 중심"
                desc="실무에서 필요한 비용 검토 포인트를 구조화"
                href="#cost"
              />
              <ClickCard
                title="법령·자료 연결"
                desc="법, 시행령, 시행규칙, 기준을 함께 정리"
                href="#law"
              />
            </div>
          </div>
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-200">
            <div className="rounded-[1.5rem] bg-slate-900 p-6 text-white">
              <p className="text-sm font-medium text-slate-300">빠른 안내</p>
              <h3 className="mt-2 text-2xl font-bold">무엇을 찾고 계신가요?</h3>

              <div className="mt-6 grid gap-3">
                {[
                  ['재건축 절차가 궁금해요', '#process'],
                  ['공사비 검증 기준을 보고 싶어요', '#cost'],
                  ['재개발과 재건축 차이를 알고 싶어요', '#overview'],
                  ['실무 체크리스트를 내려받고 싶어요', '#resources'],
                ].map(([label, href]) => (
                  <a
                    key={label}
                    href={href}
                    className="rounded-2xl bg-white/10 px-4 py-3 text-sm transition hover:bg-white/20"
                  >
                    {label}
                  </a>
                ))}
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {menu.slice(0, 6).map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="rounded-2xl border border-slate-200 px-4 py-4 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:bg-slate-50"
                >
                  {item.label}
                </a>
              ))}
      ...
      <main className="mx-auto max-w-7xl space-y-24 px-4 py-16 sm:px-6">
        <section id="overview" className="space-y-6 scroll-mt-28">
          <SectionTitle
            english="Overview"
            title="정비사업 개요"
            desc="정비사업의 기본 개념, 사업 유형, 추진 배경, 자주 쓰는 용어를 쉽게 정리해 초심자부터 실무자까지 빠르게 이해할 수 있도록 구성합니다."
          />
        ...
        <section id="process" className="space-y-6 scroll-mt-28">
          <SectionTitle english="Process" title="사업 절차" />
        ...
        <section id="cost" className="grid gap-8 scroll-mt-28 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-4">
            ...
            <p className="text-sm leading-7 text-slate-600">
              정비사업 실무에서 가장 민감한 공사비 이슈를 따로 분리해 검증 절차,
              상승 요인, 분쟁 포인트, 체크리스트 중심으로 제공합니다.
            </p>
            ...
            <div className="rounded-[1.75rem] border border-slate-200 bg-slate-900 p-6 text-white shadow-xl">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-300">
                사업 단계별 공사비 검토 항목, 증액 사유 검토, 비교견적 검토, 조합
                의사결정 지원 자료로 확장하기 좋은 메뉴입니다.
              </p>
            </div>
          </div>
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-10 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              도시정비사업 정보를 이해하기 쉽게 정리하는 실무형 정보 플랫폼
            </p>
          </div>
